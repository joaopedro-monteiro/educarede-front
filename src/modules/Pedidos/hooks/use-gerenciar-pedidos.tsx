...
interface GuiaDeRemessaFirebase {
    dataUpload: Date;
    url: string;
...
export const useGerenciarPedidos = (guiaDeRemessaId: string) => {
  ...
  const [guiaDeRemessaFirebase, setGuiaDeRemessaFirebase] =
    ...
    const fetchDataPedidos = async (guiaDeRemessaItemService: GuiaDeRemessaItemService) => {
        try {
          if (guiaDeRemessaId) {
      ...
      const fetchDataGuiaDeRemessa = async (guiaDeRemessaService: GuiaDeRemessaService) => {
        try {
    ...
    return { pedidos, guiaDeRemessa, guiaDeRemessaFirebase, fetchDataGuiaDeRemessaFirebase };
};
